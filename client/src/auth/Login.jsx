...
const Login = () => {
  ...
  if (currentUser?.user) return <Navigate to={`/${currentUser.user.role}`} replace />
      ...
      <main className="min-h-screen bg-slate-50 px-4 py-4 text-slate-900 sm:px-6 lg:px-8">
        <div className="mx-auto flex min-h-[calc(100vh-2rem)] w-full max-w-6xl flex-col">
          <header className="flex w-full items-center justify-between gap-4 py-2">
            ...
            <button type="button" className="rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm font-medium text-slate-700 transition hover:border-blue-600 hover:text-blue-600">
              Contact Support
            </button>
          </header>
            ...
            <div className="grid w-full max-w-5xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl shadow-slate-200/70 lg:grid-cols-2">
              ...
              >
                <div className="mb-8 space-y-2">
                  ...
                  <p className="text-sm leading-6 text-slate-500">Use your D&amp;C Prime account credentials to continue.</p>
                </div>
                ...
                <div className="space-y-5">
                  ...
                  <label className="block">
                    ...
                    <input type="password" required placeholder="Enter your password" onChange={(event) => setPassword(event.currentTarget.value)} value={password} autoComplete="current-password" className="h-11 w-full rounded-lg border border-slate-300 bg-white px-4 text-sm text-slate-900 outline-none transition placeholder:text-slate-400 focus:border-blue-600 focus:ring-4 focus:ring-blue-100" />
                  </label>
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button type="button" onClick={() => { setNotice(null); setShowForgotPassword(true) }} className="text-left text-sm font-medium text-blue-700 hover:text-blue-800 sm:text-right">
                      Forgot password?
                    </button>
                  </div>
                  ...
                  <button type="submit" disabled={signinMutation.isPending} className="h-11 w-full rounded-lg bg-blue-600 text-sm font-semibold tracking-wide text-white transition hover:bg-blue-700 focus:outline-none focus:ring-4 focus:ring-blue-200 disabled:cursor-not-allowed disabled:bg-blue-300">
                    {signinMutation.isPending ? 'Signing in' : 'Sign in'}
                  </button>
                </div>

                <p className="mt-8 text-center text-xs leading-5 text-slate-500">Having trouble signing in? Contact your system administrator.</p>
              </form>
            </div>
          </section>
        </div>
