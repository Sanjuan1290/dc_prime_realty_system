  ...
  return <PrintPageShell title="D&C Prime Management Report" pageOrientation="landscape">
    ...
    <ReportPage title="Management Report — Executive Summary" subtitle={reportSubtitle} section={1} totalSections={totalSections}>
      ...
      <div className="mt-3 grid grid-cols-4 gap-2">
        ...
        <MetricCard label="Commission Generated in Range" value={money(summary.commissionGenerated)} helper="Gross liability created by selected sales" />
        <MetricCard label="Commission Released in Range" value={money(summary.commissionReleased)} helper="Uses actual release date" />
        <MetricCard label={`Eligible Not Released as of ${to}`} value={money(summary.eligibleUnreleased)} helper="Eligibility based on verified cash through report end" />
        <MetricCard label={`Commission Remaining as of ${to}`} value={money(summary.commissionRemaining)} helper={`Earned on cancellation ${money(summary.commissionEarnedOnCancellation)} · Forfeited ${money(summary.commissionForfeitedOnCancellation)}`} />
      </div>
    ...
    <ReportPage title="Commission Details" subtitle={`${reportSubtitle} · Released uses actual release date; eligibility is evaluated through the report end date.`} section={6} totalSections={totalSections}>
      <div className="grid grid-cols-5 gap-2">
        <MetricCard label="Generated in Range" value={money(summary.commissionGenerated)} />
        <MetricCard label="Released in Range" value={money(summary.commissionReleased)} />
        <MetricCard label="Earned on Cancellation" value={money(summary.commissionEarnedOnCancellation)} />
        <MetricCard label="Forfeited on Cancellation" value={money(summary.commissionForfeitedOnCancellation)} />
        <MetricCard label={`Remaining as of ${to}`} value={money(summary.commissionRemaining)} emphasis />
      </div>
      <PrintTable
        headers={['Project', 'Unit', 'Buyer', 'Recipient', 'Role', 'Group', 'Stage', 'Trigger', 'Release %', 'Net Amount', 'Payment %', 'Status as of End', 'Mode', 'Scheduled', 'Actual Release', 'Released By']}
        rows={(payload.releases || []).map((row) => <tr key={row.id}>
          <Cell>{row.project}</Cell><Cell>{row.unit}</Cell><Cell>{row.buyer}</Cell><Cell strong>{row.seller}</Cell><Cell>{row.role}</Cell><Cell>{row.sellerGroup}</Cell><Cell>{row.stage}</Cell><Cell>{percent(row.triggerPercent)}</Cell><Cell>{percent(row.releasePercent)}</Cell><Cell right strong>{money(row.netAmount)}</Cell><Cell>{percent(row.paymentPercent)}</Cell><Cell>{row.status}</Cell><Cell>{titleCase(row.releaseEntryMode)}</Cell><Cell>{dateOnly(row.scheduledReleaseDate)}</Cell><Cell>{dateOnly(row.actualReleaseDate)}</Cell><Cell>{row.releasedBy}</Cell>
        </tr>)}
      />
    </ReportPage>
    ...
    <ReportPage title="Seller Performance" subtitle={`${reportSubtitle} · Ranked by net active contract value for the selected sales cohort.`} section={7} totalSections={totalSections}>
      <PrintTable
        headers={['Seller', 'Group', 'Reservations', 'Cancelled', 'Cancellation Rate', 'Gross Contracted', 'Cancelled Value', 'Net Active Sales', 'Collections', 'Refunds', 'Net Cash', 'Gross Commission', 'Released', 'Eligible Unreleased']}
        rows={(payload.sellerPerformance || []).map((row, index) => <tr key={`${row.sellerId}-${index}`}>
          <Cell strong>{row.seller}</Cell><Cell>{row.sellerGroup}</Cell><Cell>{number(row.reservations)}</Cell><Cell>{number(row.cancelledReservations)}</Cell><Cell>{percent(row.cancellationRate)}</Cell><Cell right>{money(row.grossContractedValue)}</Cell><Cell right>{money(row.cancelledValue)}</Cell><Cell right strong>{money(row.netActiveSales)}</Cell><Cell right>{money(row.collectedInRange)}</Cell><Cell right>{money(row.refundsInRange)}</Cell><Cell right strong>{money(row.netCashMovement)}</Cell><Cell right>{money(row.grossCommission)}</Cell><Cell right>{money(row.releasedCommission)}</Cell><Cell right>{money(row.eligibleUnreleased)}</Cell>
        </tr>)}
      />
    </ReportPage>
    ...
    <ReportPage title="Project Breakdown" subtitle={`${reportSubtitle} · Reconciled management view by project.`} section={8} totalSections={totalSections}>
      <PrintTable
        headers={['Project', 'Reservations', 'Cancelled Sales', 'Gross Contracted', 'Cancelled from Cohort', 'Net Active Contract', 'Cancellation Activity', 'Collections', 'Refunds', 'Net Cash', 'Cumulative Net Cash', 'Outstanding', 'Commission Generated', 'Commission Released', 'Eligible Unreleased']}
        rows={(payload.projectBreakdown || []).map((row) => <tr key={row.projectId}>
          <Cell strong>{row.project}</Cell><Cell>{number(row.reservations)}</Cell><Cell>{number(row.cohortCancelled)}</Cell><Cell right>{money(row.grossContractedValue)}</Cell><Cell right>{money(row.cohortCancelledValue)}</Cell><Cell right strong>{money(row.netActiveContractValue)}</Cell><Cell>{number(row.cancellationActivity)}</Cell><Cell right>{money(row.collectedInRange)}</Cell><Cell right>{money(row.refundsInRange)}</Cell><Cell right strong>{money(row.netCashMovement)}</Cell><Cell right>{money(row.netCumulativeCash)}</Cell><Cell right strong>{money(row.outstanding)}</Cell><Cell right>{money(row.commissionGenerated)}</Cell><Cell right>{money(row.commissionReleased)}</Cell><Cell right>{money(row.eligibleUnreleased)}</Cell>
        </tr>)}
      />
    </ReportPage>
  </PrintPageShell>
