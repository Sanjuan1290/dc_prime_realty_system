...
const ProjectRatesCell = ({ rates = [], selectedProjectId = 'all', groupType }) => {
  ...
  if (selectedProjectId !== 'all') {
    ...
    return selectedRate ? <FixedRateCard rate={selectedRate} groupType={groupType} /> : <p className="text-xs font-semibold text-slate-500">Not accredited to this project</p>
  }
  ...
  return (
    <div className="min-w-[260px]">
      ...
      <p className="mt-1 text-xs font-semibold text-slate-500">{preview.map(rateLabel).join(', ')}{extraCount ? ` +${extraCount} more` : ''}</p>
      {extraCount ? <details className="group mt-2"><summary className="inline-flex cursor-pointer list-none items-center gap-1 text-xs font-black text-blue-700">View all projects <FiChevronDown className="transition group-open:rotate-180" /></summary><div className="mt-2 max-h-44 overflow-y-auto rounded-xl border border-slate-200 bg-slate-50 p-2 shadow-inner">{rates.map((rate) => <p key={rate.lot_project_id} className="rounded-lg px-2 py-1.5 text-xs font-semibold text-slate-700 odd:bg-white">{rateLabel(rate)}</p>)}</div></details> : null}
    </div>
  )
}

const SellerGroup = ({ groupType = 'in_house' }) => {
  ...
  const groupsQuery = useQuery({ queryKey: ['seller-groups', groupType, queryString], queryFn: () => fetchJson(`/seller-groups?${queryString}`), keepPreviousData: true })
  const projectOptionsQuery = useQuery({ queryKey: ['lot-project-options'], queryFn: () => fetchJson('/projects/lot-projects/options') })
  ...
  const meta = groupsQuery.data?.meta || { active: 0, totalMembers: 0, totalAccounts: 0, accreditedProjects: 0 }
  ...
  const groupDetailsUrl = (groupId) => `${groupBasePath}/${groupId}${projectFilter !== 'all' ? `?project=${encodeURIComponent(projectFilter)}` : ''}`
  ...
  return (
    <main className="flex flex-col gap-6">
      ...
      {groupsQuery.isLoading ? <StatusAlert type="loading" message={`Loading ${config.title}...`} /> : null}
      {!groupsQuery.isLoading && groupsQuery.isFetching ? <StatusAlert type="info" message={`Refreshing ${config.title}...`} /> : null}
