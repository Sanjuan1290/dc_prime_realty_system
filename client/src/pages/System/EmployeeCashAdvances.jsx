...
const EmployeeCashAdvances = () => {
  ...
  return <main className="flex flex-col gap-6">
    ...
    <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><div className="overflow-x-auto"><table className="min-w-[1200px] w-full text-sm"><thead className="border-b border-slate-200 bg-slate-50"><tr>{['Reference', 'Employee', 'Request Date', 'Amount', 'Per Payroll', 'Remaining', 'Start Deduction', 'Status', 'Actions'].map((head) => <th key={head} className="px-4 py-3 text-left text-xs font-black uppercase tracking-wide text-slate-500">{head}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">
      {!advancesQuery.isLoading && rows.length === 0 ? <tr><td colSpan={9} className="px-6 py-14 text-center font-semibold text-slate-500">No employee cash advances match the selected filters.</td></tr> : null}
      {rows.map((advance) => <tr key={advance.employee_cash_advance_id} className="hover:bg-slate-50"><td className="px-4 py-4 font-black text-blue-700">{advance.reference_number}</td><td className="px-4 py-4"><p className="font-black">{advance.full_name}</p><p className="text-xs text-slate-500">{advance.employee_code} · {advance.department || '-'}</p></td><td className="px-4 py-4 text-slate-600">{advance.request_date}</td><td className="px-4 py-4 font-black">{money(advance.amount)}</td><td className="px-4 py-4">{money(advance.deduction_per_payroll)}<p className="text-xs text-slate-500">{advance.installment_count} installment(s)</p></td><td className="px-4 py-4 font-black text-amber-700">{money(advance.remaining_balance)}<p className="text-xs text-slate-500">Deducted: {money(advance.total_deducted)}</p></td><td className="px-4 py-4 text-slate-600">{advance.start_deduction_date}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-black capitalize ${statusTone[advance.cash_advance_status] || statusTone.pending}`}>{advance.cash_advance_status}</span></td><td className="px-4 py-4"><div className="flex gap-2"><button type="button" onClick={() => setDetailsId(advance.employee_cash_advance_id)} className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-black text-slate-700 hover:bg-slate-50"><FiEye />View</button>{canManage && advance.cash_advance_status === 'pending' ? <button type="button" onClick={() => openEdit(advance)} className="h-9 rounded-lg border border-blue-200 bg-blue-50 px-3 text-xs font-black text-blue-700 hover:bg-blue-100">Edit</button> : null}</div></td></tr>)}
