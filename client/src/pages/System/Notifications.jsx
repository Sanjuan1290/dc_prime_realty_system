...
const getDocumentRecordStatus = (item = {}) => {
  if (Number(item.totalDocuments || 0) <= 0) {
    return { label: 'No Documents', className: 'bg-slate-100 text-slate-600' }
  }
  if (Number(item.pendingDocuments || 0) > 0) {
    return { label: 'Incomplete', className: 'bg-red-50 text-red-700' }
  }
  if (Number(item.awaitingApprovalDocuments || 0) > 0) {
    return { label: 'For Review', className: 'bg-amber-50 text-amber-700' }
  }
  ...
  return (
    <main className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
        <PageHeader title="Notifications" description="Payment reminders and document status records across all lot projects." icon={FiBell} />
      ...
      {tab === 'payments' ? (
        ...
        <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
          <SummaryCard label="Units" value={documentSummary.totalUnits} icon={FiFileText} helper="Active, fully paid, or pending cancellation" />
          <SummaryCard label="Pending Documents" value={documentSummary.pendingDocuments} icon={FiAlertTriangle} tone="red" helper={`${documentSummary.clientActionDocuments || 0} client document${Number(documentSummary.clientActionDocuments || 0) === 1 ? '' : 's'} can be included in an email`} />
          <SummaryCard label="Missing Documents" value={documentSummary.missingDocuments} icon={FiClock} tone="amber" helper="Not uploaded" />
          <SummaryCard label="Rejected Documents" value={documentSummary.rejectedDocuments} icon={FiAlertTriangle} tone="red" helper="Needs resubmission" />
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        <div className="flex flex-col gap-4 border-b border-slate-200 p-5 lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm font-semibold text-slate-500">{tab === 'payments' ? 'Send reminders for dues within 7 days and overdue schedules.' : 'Monitor all listing documents. Email actions include only client-responsibility documents that are missing or rejected.'}</p>
          </div>
        ...
        {activeQuery.isLoading ? <div className="flex min-h-[280px] items-center justify-center"><StatusAlert type="loading" message={`Loading ${tab === 'payments' ? 'payment' : 'document'} notifications...`} /></div> : records.length === 0 ? (
          ...
          <>
            <div className="overflow-x-auto"><table className="min-w-[1380px] w-full divide-y divide-slate-200 text-sm"><thead className="bg-slate-50"><tr>{['Project', 'Unit', 'Buyer', 'Email / Contact', 'Submitted / Total', 'Approved', 'Awaiting', 'Missing', 'Rejected', 'Status', 'Last Email', 'Actions'].map((header) => <th key={header} className="px-4 py-3 text-left text-xs font-black uppercase tracking-wide text-slate-500">{header}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">{paginatedRecords.map((item) => { const documentStatus = getDocumentRecordStatus(item); return <tr key={item.listingId} className="align-top hover:bg-slate-50"><td className="px-4 py-4 font-black">{item.projectName}</td><td className="px-4 py-4 font-black text-blue-700">{item.unitId}</td><td className="px-4 py-4 font-semibold text-slate-700">{item.buyerName}</td><td className="px-4 py-4 font-semibold text-slate-600"><p>{item.buyerEmail || '-'}</p><p className="text-xs">{item.buyerContactNumber || '-'}</p></td><td className="px-4 py-4 font-black text-blue-700">{item.submittedDocuments} / {item.totalDocuments}</td><td className="px-4 py-4 font-black text-emerald-700">{item.approvedDocuments}</td><td className="px-4 py-4 font-black text-amber-700">{item.awaitingApprovalDocuments}</td><td className="px-4 py-4 font-black text-red-700">{item.missingDocuments}</td><td className="px-4 py-4 font-black text-red-700">{item.rejectedDocuments}</td><td className="px-4 py-4"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-black ${documentStatus.className}`}>{documentStatus.label}</span></td><td className="px-4 py-4"><span className={`inline-flex rounded-full border px-3 py-1 text-xs font-black ${statusStyles[item.lastDocumentNotificationStatus] || 'border-slate-200 bg-slate-50 text-slate-600'}`}>{getLastStatusLabel(item.lastDocumentNotificationStatus)}</span>{item.lastDocumentNotificationAt ? <p className="mt-1 text-xs font-semibold text-slate-500">{item.lastDocumentNotificationAt}</p> : null}</td><td className="px-4 py-4"><div className="flex flex-wrap gap-2">{canManage ? <button type="button" onClick={() => handleDocumentSend(item)} disabled={isBusy || !item.buyerEmail || item.clientActionDocuments <= 0} title={item.clientActionDocuments > 0 ? `${item.clientActionDocuments} client document${item.clientActionDocuments === 1 ? '' : 's'} need action` : 'No missing or rejected client-responsibility documents'} className="inline-flex h-9 items-center gap-2 rounded-lg bg-blue-600 px-3 text-xs font-black text-white hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-50"><FiMail /> Send Client Email</button> : null}<Link to={item.listingPath} className="inline-flex h-9 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-xs font-black text-slate-700 hover:bg-slate-50"><FiFileText /> Review</Link></div></td></tr>})}</tbody></table></div>
            <Pagination total={records.length} page={currentPage} pageSize={pageSize} onPageChange={setPage} onPageSizeChange={(size) => { setPageSize(size); setPage(1) }} />
          </>
        )}
      </section>
