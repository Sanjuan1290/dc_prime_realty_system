...
const formatEventDateRange = (event) => {
  ...
  if (!start) return '—'
  if (!end || start === end) return formatCompactDate(start)
  return `${formatCompactDate(start)} – ${formatCompactDate(end)}`
}

const attendanceTreatmentLabel = {
  record_only: 'Attendance Record Only',
...
const Attendance = () => {
  ...
  const dayMutation = useMutation({
    ...
    onMutate: () => setAlert({ type: 'loading', message: 'Saving day details...' }),
  ...
  const deleteEventMutation = useMutation({
    ...
    onSuccess: (result) => {
      setAlert({ type: 'success', message: result.message || 'Company event removed.' })
      setDayType('regular')
      setDayNotes('')
      invalidateAttendance()
    },
  ...
  const confirmRemoveCompanyEvent = (event) => {
    const range = formatEventDateRange(event)
    const confirmed = window.confirm(
      `Remove company event \"${event.event_name}\"?\n\nThis removes the whole event (${range}) and the attendance records generated by it. To remove only one date from a multi-day event, choose Edit Whole Event and change the event date range.`
    )
    ...
    <main className="flex flex-col gap-6">
      ...
      <section className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 text-sm">
            ...
            <div className="rounded-xl bg-slate-50 p-3"><p className="text-xs font-black uppercase text-slate-400">Regular Working Hours</p><p className="mt-1 font-black text-slate-900">{Number(attendanceSettings.regularWorkingMinutes || 600) / 60} hrs</p></div>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.65fr_.85fr]">
        ...
        <aside className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-xs font-black uppercase tracking-wide text-slate-400">Date Details</p>
              <h2 className="mt-1 text-lg font-black text-slate-950">{formatSelectedDate(selectedDate)}</h2>
            </div>
            {hasCompanyEvent ? (
              <span className="rounded-full bg-violet-600 px-3 py-1 text-xs font-black text-white">COMPANY EVENT</span>
            ) : dayType === 'double_pay' ? (
              <span className="rounded-full bg-amber-500 px-3 py-1 text-xs font-black text-white">DOUBLE PAY</span>
            ) : null}
          </div>
              ...
              {hasCompanyEvent ? (
                <>
                  <div className="grid gap-2">
                    <span className="text-sm font-black text-slate-700">Day Type</span>
                    <div className="flex h-11 items-center rounded-xl border border-violet-200 bg-violet-50 px-3 text-sm font-black text-violet-800">
                      Company Event
                    </div>
                    <p className="text-xs font-semibold leading-5 text-slate-500">
                      This date belongs to a company event. Edit the event itself to change its dates, participants, attendance treatment, or pay classification.
                    </p>
                  </div>

                  <div className="grid gap-3">
                    {events.map((event) => (
                      <div key={event.attendance_event_id} className="rounded-2xl border border-violet-200 bg-violet-50 p-4">
                        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                          <div className="min-w-0">
                            <p className="text-lg font-black text-slate-950">{event.event_name}</p>
                            <p className="mt-1 text-sm font-black text-violet-700">{formatEventDateRange(event)}</p>
                            <p className="mt-1 text-xs font-semibold text-slate-500">
                              {event.location || 'No location'} · {Number(event.participant_count || 0)} participant{Number(event.participant_count || 0) === 1 ? '' : 's'}
                            </p>
                          </div>
                          {event.day_type === 'double_pay' ? (
                            <span className="shrink-0 rounded-full bg-amber-100 px-2 py-1 text-[10px] font-black text-amber-800">DOUBLE PAY</span>
                          ) : null}
                        </div>

                        <div className="mt-3 grid gap-2 rounded-xl border border-violet-100 bg-white p-3 text-xs font-semibold text-slate-600">
                          <p><span className="font-black text-slate-700">Attendance:</span> {attendanceTreatmentLabel[event.attendance_treatment] || event.attendance_treatment || '—'}{event.event_time_in ? ` · ${formatTime(event.event_time_in)}–${formatTime(event.event_time_out)}` : ''}</p>
                          <p><span className="font-black text-slate-700">Pay Classification:</span> {dayTypeLabel[event.day_type] || event.day_type || 'Regular Day'}</p>
                          {event.notes ? <p><span className="font-black text-slate-700">Notes:</span> {event.notes}</p> : null}
                        </div>

                        <div className="mt-4 flex flex-col gap-2 sm:flex-row">
                          <button
                            type="button"
                            onClick={() => openCompanyEvent(event)}
                            className="inline-flex h-10 flex-1 items-center justify-center gap-2 rounded-xl bg-violet-600 px-4 text-xs font-black text-white transition hover:bg-violet-700"
                          >
                            <FiEdit2 />Edit Whole Event
                          </button>
                          <button
                            type="button"
                            onClick={() => confirmRemoveCompanyEvent(event)}
                          ...
                          >
                            <FiTrash2 />Remove Company Event
                          </button>
                        </div>

                        <p className="mt-3 text-[11px] font-semibold leading-5 text-violet-700">
                          To remove only {formatCompactDate(selectedDate)} from this event, use Edit Whole Event and adjust the start or end date.
                        </p>
                      </div>
                    ))}
                  </div>
                ...
                <>
                  <label className="grid gap-2">
                    <span className="text-sm font-black text-slate-700">Day Type</span>
                    <select value={dayType} onChange={(e) => setDayType(e.target.value)} className="h-11 rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold">
                      <option value="regular">Regular Day</option>
                      <option value="double_pay">Double Pay Day</option>
                  ...
                  {dayType !== 'company_event' ? (
                    <label className="grid gap-2">
                      <span className="text-sm font-black text-slate-700">Note <span className="font-semibold text-slate-400">(optional)</span></span>
                      <textarea rows={3} value={dayNotes} onChange={(e) => setDayNotes(e.target.value)} placeholder="Optional note for this date" className="rounded-xl border border-slate-300 bg-white px-3 py-3 text-sm font-semibold outline-none focus:border-blue-400 focus:ring-4 focus:ring-blue-50" />
                    </label>
                  ) : (
                    <div className="rounded-2xl border border-violet-200 bg-violet-50 p-4">
                      <p className="text-sm font-black text-violet-900">Set up a Company Event</p>
                      <p className="mt-1 text-xs font-semibold leading-5 text-violet-700">
                        Company Event is managed as one event record. You can choose a start/end date and edit all participants together later.
                      </p>
                    </div>
                  )}
                  ...
                  <button
                    type="button"
                    onClick={saveDateDetails}
                    disabled={dayMutation.isPending}
                    className={`h-11 rounded-xl px-4 text-sm font-black text-white disabled:opacity-60 ${dayType === 'company_event' ? 'bg-violet-600' : 'bg-slate-950'}`}
                  >
                    {dayType === 'company_event' ? 'Set Up Company Event' : 'Save Changes'}
                  </button>
                </>
              )}
            </div>
            ...
            <div className="mt-5 grid gap-3">
              {events.map((event) => (
                <div key={event.attendance_event_id} className="rounded-2xl border border-violet-200 bg-violet-50 p-4">
                  ...
                  <p className="mt-1 text-sm font-black text-violet-700">{formatEventDateRange(event)}</p>
                  <p className="mt-2 text-xs font-semibold text-slate-600">{event.location || 'No location'} · {Number(event.participant_count || 0)} participant{Number(event.participant_count || 0) === 1 ? '' : 's'}</p>
                  <p className="mt-1 text-xs font-semibold text-slate-600">{attendanceTreatmentLabel[event.attendance_treatment] || event.attendance_treatment || '—'} · {dayTypeLabel[event.day_type] || event.day_type || 'Regular Day'}</p>
                </div>
              ))}
            </div>
          ) : (
            <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <p className="text-sm font-black text-slate-800">{dayTypeLabel[dayType] || dayType}</p>
              {dayNotes ? <p className="mt-2 text-sm font-semibold text-slate-600">{dayNotes}</p> : null}
            </div>
          )}
        </aside>
      </section>
