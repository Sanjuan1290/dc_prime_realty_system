...
const Attendance = () => {
  ...
  }, [attendanceQuery.data?.day?.attendance_date, attendanceQuery.data?.day?.day_type, attendanceQuery.data?.day?.notes])
  ...
  const scanMutation = useMutation({
    ...
    onSuccess: (result) => {
      ...
      setBarcode('')
      if (data.date) { setSelectedDate(data.date); setPage(1) }
  ...
  const dayMutation = useMutation({
    mutationFn: () => useFetchPut(`/attendance/day/${selectedDate}`, { day_type: dayType, notes: dayNotes }, { confirmationHandled: 'compact' }),
    onMutate: () => setAlert({ type: 'loading', message: 'Saving day classification...' }),
  ...
  return (
    <main className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
        <PageHeader title="Attendance" description="Barcode Time In/Time Out, manual corrections, company events, and attendance history." icon={FiClock} />
        <div className="flex flex-wrap gap-2">
          <button type="button" onClick={() => attendanceQuery.refetch()} className="inline-flex h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700"><FiRefreshCw className={attendanceQuery.isFetching ? 'animate-spin' : ''} />Refresh</button>
          {canManage ? <><button type="button" onClick={() => { setCorrectionRecord(null); setShowCorrection(true) }} className="inline-flex h-11 items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-4 text-sm font-black text-blue-700"><FiPlus />Manual Attendance</button><button type="button" onClick={() => { setEventRecord(null); setShowEvent(true) }} className="inline-flex h-11 items-center gap-2 rounded-xl bg-violet-600 px-4 text-sm font-black text-white"><FiCalendar />Company Event</button></> : null}
        </div>
      </div>
      ...
      <section className="grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
          ...
          <div className="grid gap-5 p-5 sm:p-6">
            ...
            <div className="rounded-2xl border border-blue-100 bg-blue-50 p-4">
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <button type="button" onClick={() => setShowScanner(true)} className="inline-flex h-12 items-center justify-center gap-2 rounded-xl border border-blue-200 bg-white px-4 text-sm font-black text-blue-700"><FiCamera />Camera</button>
              ...
              <p className="mt-2 text-xs font-semibold text-blue-700">Camera unavailable? Enter the code manually or use a physical barcode scanner while the field is focused.</p>
            </div>
        ...
        <div className="grid content-start gap-4">
          <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><p className="text-xs font-black uppercase tracking-wide text-slate-400">Automatic Time Out</p><p className="mt-2 text-3xl font-black text-slate-950">{formatTime(defaultTimeOut)}</p><p className="mt-2 text-sm font-semibold leading-6 text-slate-500">Employees who Time In but forget to Time Out are closed automatically at this time. Change it in System Settings.</p></section>
          <section className={`rounded-3xl border p-5 shadow-sm ${dayType === 'double_pay' ? 'border-amber-200 bg-amber-50' : 'border-slate-200 bg-white'}`}><div className="flex items-center justify-between gap-3"><div><p className="text-xs font-black uppercase tracking-wide text-slate-400">Selected Day</p><p className="mt-1 font-black text-slate-950">{selectedDate}</p></div>{dayType === 'double_pay' ? <span className="rounded-full bg-amber-500 px-3 py-1 text-xs font-black text-white">DOUBLE PAY</span> : null}</div>{canManage ? <div className="mt-4 grid gap-3"><select value={dayType} onChange={(e) => setDayType(e.target.value)} className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold"><option value="regular">Regular Day</option><option value="double_pay">Double Pay Day</option><option value="regular_holiday">Regular Holiday</option><option value="special_holiday">Special Holiday</option></select><input value={dayNotes} onChange={(e) => setDayNotes(e.target.value)} placeholder="Optional note" className="h-10 rounded-xl border border-slate-300 bg-white px-3 text-sm font-semibold" /><button type="button" onClick={() => dayMutation.mutate()} disabled={dayMutation.isPending} className="h-10 rounded-xl bg-slate-950 px-4 text-sm font-black text-white">Save Classification</button></div> : <p className="mt-3 text-sm font-semibold text-slate-600">{dayTypeLabel[dayType] || dayType}</p>}</section>
        </div>
      </section>
      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto"><table className="min-w-[1100px] w-full text-sm"><thead className="border-b border-slate-200 bg-slate-50"><tr>{['Employee', 'Department', 'Time In', 'Time Out', 'Source', 'Day Type', 'Status', 'Actions'].map((head) => <th key={head} className="px-4 py-3 text-left text-xs font-black uppercase tracking-wide text-slate-500">{head}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">
          ...
          {!attendanceQuery.isLoading && rows.length === 0 ? <tr><td colSpan={8} className="px-6 py-16 text-center"><p className="font-black text-slate-800">No attendance records for this day</p><p className="mt-1 text-sm font-semibold text-slate-500">Barcode scans, manual entries, or company events will appear here.</p></td></tr> : null}
          {rows.map((record) => {
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between gap-3"><div><h2 className="font-black text-slate-950">Company Events · {selectedDate}</h2><p className="text-xs font-semibold text-slate-500">Off-site attendance remains visible in the log instead of being disguised as barcode scans.</p></div>{canManage ? <button type="button" onClick={() => { setEventRecord(null); setShowEvent(true) }} className="inline-flex h-10 items-center gap-2 rounded-xl bg-violet-600 px-4 text-sm font-black text-white"><FiPlus />Add Event</button> : null}</div><div className="mt-4 grid gap-3 lg:grid-cols-2">{events.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 p-6 text-center text-sm font-semibold text-slate-500">No company events overlap this date.</div> : events.map((event) => <article key={event.attendance_event_id} className="rounded-2xl border border-violet-100 bg-violet-50 p-4"><div className="flex items-start justify-between gap-3"><div><p className="font-black text-slate-950">{event.event_name}</p><p className="mt-1 text-xs font-semibold text-slate-600">{String(event.start_date).slice(0, 10)}{String(event.end_date).slice(0, 10) !== String(event.start_date).slice(0, 10) ? ` – ${String(event.end_date).slice(0, 10)}` : ''}{event.location ? ` · ${event.location}` : ''}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-violet-700">{event.participant_count} participant{event.participant_count === 1 ? '' : 's'}</span></div><div className="mt-3 flex flex-wrap gap-2"><span className="rounded-full bg-white px-3 py-1 text-xs font-black text-slate-600">{event.attendance_treatment === 'record_only' ? 'Attendance Record Only' : event.attendance_treatment === 'full_day' ? 'Full Day' : 'Custom Time'}</span>{event.day_type !== 'regular' ? <span className="rounded-full bg-amber-100 px-3 py-1 text-xs font-black text-amber-800">{dayTypeLabel[event.day_type]}</span> : null}</div>{event.participants?.length ? <p className="mt-3 text-xs font-semibold leading-5 text-slate-600">{event.participants.slice(0, 6).map((item) => item.full_name).join(', ')}{event.participants.length > 6 ? ` +${event.participants.length - 6} more` : ''}</p> : null}{canManage ? <div className="mt-4 flex gap-2"><button type="button" onClick={() => { setEventRecord(event); setShowEvent(true) }} className="inline-flex h-9 items-center gap-2 rounded-lg bg-white px-3 text-xs font-black text-violet-700"><FiEdit2 />Edit Event</button><button type="button" onClick={() => { if (window.confirm(`Remove company event "${event.event_name}" and its generated attendance records?`)) deleteEventMutation.mutate(event) }} className="h-9 rounded-lg bg-red-50 px-3 text-xs font-black text-red-700">Remove</button></div> : null}</article>)}</div></section>
