          ...
          <table className="min-w-[1300px] divide-y divide-slate-200 text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredListings.map((listing) => <tr key={listing.id} className="transition hover:bg-slate-50">
                ...
                <td className="px-4 py-4"><Link to={`/bailenProject/listings/${listing.unitCode}`} className="inline-flex h-9 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-black text-slate-700 transition hover:bg-slate-50"><FiEye className="h-4 w-4" />Details</Link></td>
              </tr>)}
            </tbody>
          </table>
        </div>
