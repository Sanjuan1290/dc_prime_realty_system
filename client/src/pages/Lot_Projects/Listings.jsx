      ...
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-[1650px] divide-y divide-slate-200 text-sm">
            <thead className="bg-slate-50"><tr>{['Unit ID','Old Unit IDs','Cadastral Lots','Lot Type','Area','Installment / SQM','Cash / SQM','Installment TCP','Cash TCP','LMF Rate','RS','Buyer','Documents','Status','Actions'].map((head) => <th key={head} className="px-4 py-3 text-left text-xs font-black uppercase tracking-wider text-slate-500">{head}</th>)}</tr></thead>
            <tbody className="divide-y divide-slate-100">
              {isLoading ? <tr><td colSpan={15} className="px-4 py-10 text-center text-sm font-semibold text-slate-500">Loading listings...</td></tr> : null}
              {!isLoading && listings.length === 0 ? <tr><td colSpan={15} className="px-4 py-10 text-center text-sm font-semibold text-slate-500">No listings found. Add your first listing for this project.</td></tr> : null}
