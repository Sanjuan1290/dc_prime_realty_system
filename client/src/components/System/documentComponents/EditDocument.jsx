...
const EditDocument = ({ document, onClose, onSaved }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    document_is_reusable: document?.document_is_reusable ? "yes" : "no",
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="w-full max-w-2xl overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        ...
        <div className="flex flex-col gap-5 px-6 py-5">
          {mutation.isPending ? <StatusAlert type="loading" message="Saving document changes..." /> : null}
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
            <label className="flex flex-col gap-2">
              <span className="text-sm font-semibold text-slate-700">Reusable Across Units</span>
              <select value={formData.document_is_reusable} onChange={(event) => handleChange("document_is_reusable", event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-900 shadow-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-200 bg-slate-50 px-6 py-4">
          ...
          <button type="submit" disabled={mutation.isPending} className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-blue-700 active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-blue-300">{mutation.isPending ? "Saving..." : "Save Changes"}</button>
        </div>
      </form>
    </div>
  );
