...
const isReviewCancelled = (error) =>
  Number(error?.status || 0) === 499 || /review cancelled/i.test(String(error?.message || ''))

const getDocumentStatusLabel = (status) =>
  String(status || 'active').toLowerCase() === 'inactive' ? 'Inactive' : 'Active'

const buildTemplateReviewPayload = (formData, selectedDocuments) => ({
  templateInformation: {
    templateName: formData.template_name,
    templateDescription: formData.template_description,
    templateStatus: formData.template_status,
  },
...
const EditDocumentTemplate = ({ template, documents = [], templateDocuments = [], onClose, onSaved }) => {
  ...
  const availableDocuments = useMemo(() => {
    ...
    return documents.filter((document) => {
      ...
      return document.document_name?.toLowerCase().includes(keyword)
        || document.document_description?.toLowerCase().includes(keyword);
    });
  }, [documents, search, selectedDocumentIds]);

  const mutation = useMutation({
    mutationFn: () => useFetchPut(
      `/documents/editTemplate/${template.template_id}`,
      {
        ...formData,
        document_ids: selectedDocuments.map((document) => document.document_id),
      ...
      },
      {
        review: {
          ...
          confirmLabel: 'Confirm & Save Template',
          description: 'Verify the template information and every selected document before saving the changes.',
          summary: formData.template_name || template?.template_name || 'Document template',
    ...
    onSuccess: (data) => {
      ...
      onClose();
      onSaved?.(data?.message || "Template saved successfully.");
    },
    onError: (error) => {
      if (isReviewCancelled(error)) {
        setErrorMessage("");
        setNotice({ type: 'info', message: 'Final review closed. You can continue editing the template; nothing was saved.' });
        return;
      }
      setNotice(null);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-xl font-bold text-slate-950">Edit Document Template</h3>
            <p className="mt-1 text-sm text-slate-500">Edit the checklist name, status, and selected documents.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
            ...
            <label className="flex flex-col gap-2">
              ...
              <select value={formData.template_status} onChange={(event) => handleChange("template_status", event.target.value)} className="h-11 rounded-xl border border-slate-200 bg-white px-4 text-sm text-slate-900 shadow-sm outline-none transition focus:border-blue-500 focus:ring-4 focus:ring-blue-100">
          ...
          <TemplateDocuments documents={availableDocuments} selectedDocuments={selectedDocuments} search={search} setSearch={setSearch} setSelectedDocuments={setSelectedDocuments} />
          {mutation.isPending ? <StatusAlert type="loading" message="Preparing template review..." className="mt-4" /> : null}
          {notice ? <StatusAlert type={notice.type} message={notice.message} className="mt-4" /> : null}
        ...
        <div className="flex items-center justify-end gap-2 border-t border-slate-200 bg-white px-6 py-4">
          ...
          <button type="submit" disabled={mutation.isPending} className="rounded-xl bg-blue-600 px-5 py-2.5 text-sm font-bold text-white shadow-sm transition hover:bg-blue-700 active:scale-[0.98] disabled:cursor-not-allowed disabled:bg-blue-300">{mutation.isPending ? "Opening Review..." : "Save Changes"}</button>
        </div>
      </form>
    </div>
  );
...
const TemplateDocuments = ({ documents, selectedDocuments, search, setSearch, setSelectedDocuments }) => {
  const handleAddDocument = (document) => setSelectedDocuments((prev) => [...prev, document]);
  const handleRemoveDocument = (documentId) => setSelectedDocuments((prev) => prev.filter((document) => document.document_id !== documentId));
  const noAvailableMessage = search.trim()
    ? 'No active documents match your search.'
    : selectedDocuments.length
      ? 'No additional active documents are available to add.'
      : 'No active documents are available in the Document Library.';

  return (
    <div className="mt-5 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
      <div>
        <h3 className="text-lg font-bold text-slate-950">Template Documents</h3>
        <p className="mt-1 text-sm text-slate-500">Add documents to this template. Projects can still customize the list later.</p>
      </div>
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        <div className="mb-3">
          <h3 className="text-sm font-bold text-slate-950">Add Existing Documents</h3>
          <p className="mt-1 text-xs text-slate-500">Search active documents and add them to this template.</p>
        </div>
        <div className="relative mb-4">
          <FiSearch className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" />
          <input type="text" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search document library..." className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" />
        </div>
        {documents.length === 0 ? (
          <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm text-slate-500">{noAvailableMessage}</p>
        ) : (
          <div className="grid max-h-[220px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2">
            {documents.map((document) => (
              <div key={document.document_id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3">
                ...
                <button type="button" onClick={() => handleAddDocument(document)} className="shrink-0 rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-blue-700 active:scale-[0.98]">Add</button>
              </div>
            ))}
          </div>
        )}
      ...
      {selectedDocuments.length === 0 ? (
        <div className="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-5">
          <p className="text-sm text-slate-500">No documents added to this template yet.</p>
        </div>
      ) : (
          ...
          {selectedDocuments.map((document) => (
            <div key={document.document_id} className="grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-5 md:items-center">
              ...
              <div className="flex flex-col gap-1">
                ...
                <p className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm capitalize text-slate-700">{document.document_status}</p>
              </div>
              <div className="flex md:justify-end">
                <button type="button" onClick={() => handleRemoveDocument(document.document_id)} className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-bold text-red-700 transition hover:bg-red-100 active:scale-[0.98]">Remove</button>
              </div>
            </div>
          ))}
        </div>
