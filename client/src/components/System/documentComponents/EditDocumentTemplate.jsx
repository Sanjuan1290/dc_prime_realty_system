...
const TemplateDocuments = ({ documents, selectedDocuments, search, setSearch, setSelectedDocuments }) => {
  ...
  return <div className="mt-5 flex flex-col gap-4 rounded-2xl border border-slate-200 bg-slate-50 p-4"><div><h3 className="text-lg font-bold text-slate-950">Template Documents</h3><p className="mt-1 text-sm text-slate-500">Add documents to this template. Projects can still customize the list later.</p></div><div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="mb-3"><h3 className="text-sm font-bold text-slate-950">Add Existing Documents</h3><p className="mt-1 text-xs text-slate-500">Search active documents and add them to this template.</p></div><div className="relative mb-4"><FiSearch className="pointer-events-none absolute left-3 top-1/2 h-5 w-5 -translate-y-1/2 text-slate-400" /><input type="text" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search document library..." className="h-11 w-full rounded-xl border border-slate-200 bg-white pl-10 pr-4 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-blue-500 focus:ring-4 focus:ring-blue-100" /></div>{documents.length === 0 ? <p className="rounded-xl border border-dashed border-slate-300 bg-slate-50 px-4 py-5 text-sm text-slate-500">No available documents found.</p> : <div className="grid max-h-[220px] grid-cols-1 gap-3 overflow-y-auto pr-1 md:grid-cols-2">{documents.map((document) => <div key={document.document_id} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3"><div className="min-w-0"><h3 className="truncate text-sm font-bold text-slate-950">{document.document_name}</h3><p className="line-clamp-1 text-xs text-slate-500">{document.document_description || "No description"}</p></div><button type="button" onClick={() => handleAddDocument(document)} className="shrink-0 rounded-xl bg-blue-600 px-4 py-2 text-sm font-bold text-white transition hover:bg-blue-700 active:scale-[0.98]">Add</button></div>)}</div>}</div>{selectedDocuments.length === 0 ? <div className="rounded-xl border border-dashed border-slate-300 bg-white px-4 py-5"><p className="text-sm text-slate-500">No documents added to this template yet.</p></div> : <div className="flex flex-col gap-3">{selectedDocuments.map((document) => <div key={document.document_id} className="grid grid-cols-1 gap-3 rounded-2xl border border-slate-200 bg-white p-4 md:grid-cols-5 md:items-center"><div className="flex min-w-0 flex-col gap-1 md:col-span-2"><h3 className="truncate text-sm font-bold text-slate-950">{document.document_name}</h3><p className="line-clamp-1 text-xs text-slate-500">{document.document_description || "No description"}</p></div><div className="flex flex-col gap-1"><span className="text-xs font-semibold text-slate-500">Requirement</span><p className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700">{document.document_is_required ? "Required" : "Optional"}</p></div><div className="flex flex-col gap-1"><span className="text-xs font-semibold text-slate-500">Status</span><p className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm capitalize text-slate-700">{document.document_status}</p></div><div className="flex md:justify-end"><button type="button" onClick={() => handleRemoveDocument(document.document_id)} className="rounded-xl border border-red-200 bg-red-50 px-4 py-2 text-sm font-bold text-red-700 transition hover:bg-red-100 active:scale-[0.98]">Remove</button></div></div>)}</div>}</div>;
};
