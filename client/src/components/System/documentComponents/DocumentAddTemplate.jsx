...
const DocumentAddTemplate = ({ setShowAddTemplateModal, documents = [], onSaved }) => {
  const queryClient = useQueryClient();
  const [search, setSearch] = useState("");
  const [errorMessage, setErrorMessage] = useState("");
  const [notice, setNotice] = useState(null);
  const [formData, setFormData] = useState({ template_name: "", template_description: "", template_status: "active" });
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data) => {
      ...
      onSaved?.(data?.message || "Template created successfully.");
    },
    onError: (error) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4 backdrop-blur-sm">
      <form onSubmit={handleSubmit} className="flex max-h-[92vh] w-full max-w-6xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            <h3 className="text-xl font-bold text-slate-950">Add Document Template</h3>
            <p className="mt-1 text-sm text-slate-500">Create a checklist group from existing document library items.</p>
          </div>
          <button type="button" onClick={() => setShowAddTemplateModal(false)} className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 active:scale-[0.98]">
