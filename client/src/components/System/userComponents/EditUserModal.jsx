...
const EditUserModal = ({ setShowEditUser, selectedUser, onSaved }) => {
  ...
  useEffect(() => {
    ...
    setProjectRates(normalizeProjectRates(lotProjects, selectedUser?.project_rates?.length ? selectedUser.project_rates : projectRates, getRoleDefaultRate(form.role)));
  ...
  const createMutation = useMutation({
    mutationFn: () => useFetchPut(`/user/editUser/${selectedUser.id}`, {
      ...form,
      ...
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          <div>
            ...
            <p className="text-sm text-slate-500">Update account details and seller hierarchy.</p>
          </div>

          <button type="button" onClick={() => setShowEditUser(false)} disabled={createMutation.isPending} className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-60">
        ...
        <div className="overflow-y-auto px-6 py-5">
          <div className="grid gap-5">
            {createMutation.isPending ? <StatusAlert type="loading" message="Saving user changes and seller rates..." /> : null}
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-500"><p className="font-bold text-slate-700">Password</p><p>Use Reset Password from the users table if needed.</p></div>
            </div>
        ...
        <div className="flex flex-col-reverse gap-2 border-t border-slate-200 bg-slate-50 px-6 py-4 sm:flex-row sm:justify-end">
          <button type="button" onClick={() => setShowEditUser(false)} disabled={createMutation.isPending} className="h-11 rounded-xl border border-slate-200 bg-white px-5 text-sm font-bold text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-60">Cancel</button>
          <button type="button" disabled={createMutation.isPending} onClick={handleSubmit} className="h-11 rounded-xl bg-blue-600 px-5 text-sm font-bold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:bg-blue-300">{createMutation.isPending ? "Creating..." : "Edit User"}</button>
        </div>
      </div>
    </div>
  );
