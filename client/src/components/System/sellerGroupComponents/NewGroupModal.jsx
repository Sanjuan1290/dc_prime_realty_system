...
const NewGroupModal = ({ setShowNewGroupModal, onSaved }) => {
  ...
  const createMutation = useMutation({
    mutationFn: () => useFetchPost("/seller-groups/create", { ...form, project_rates: projectRates }),
    onSuccess: (data) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4">
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex items-center justify-between border-b border-slate-200 px-6 py-4">
          ...
          <button type="button" onClick={() => setShowNewGroupModal(false)} disabled={createMutation.isPending} className="flex h-10 w-10 items-center justify-center rounded-xl text-slate-500 transition hover:bg-slate-100 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-60"><FiX className="h-5 w-5" /></button>
        </div>

        <div className="overflow-y-auto px-6 py-5">
          <div className="grid gap-5">
            {createMutation.isPending ? <StatusAlert type="loading" message="Creating seller group and saving project rates..." /> : null}
