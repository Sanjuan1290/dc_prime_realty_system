...
const EditListingDocumentsModal = ({ selectedDocuments = [], setSelectedDocuments, libraryDocuments = [], projectDefaultDocuments = [], onClose }) => {
  ...
  const isDocumentAdded = (documentId) => documents.some((document) => Number(document.id) === Number(documentId))
  ...
  const removeDocument = (documentId) => {
    ...
    window.setTimeout(() => {
      setDocuments((current) => current.filter((document) => Number(document.id) !== Number(documentId)))
      setDeletingId(null)
  ...
  const updateDocument = (documentId, key, value) => {
    setDocuments((current) => current.map((document) => Number(document.id) === Number(documentId) ? { ...document, [key]: value } : document))
      ...
      <div className="flex h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm font-semibold text-slate-500">These requirements will be saved together with the new listing. Leave empty to use project defaults when saving.</p>
              </div>
          ...
          <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
            ...
            <div className="grid max-h-[235px] gap-2 overflow-y-auto pr-1 md:grid-cols-2">
              ...
              {filteredLibraryDocuments.map((document) => {
                ...
                return (
                  <div key={document.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 transition hover:border-blue-200 hover:bg-blue-50/40">
          ...
          {documents.length ? (
            <section className="mt-4 space-y-2">
              {documents.map((document) => {
                const isDeleting = Number(deletingId) === Number(document.id)
                return (
                  <div key={document.id} className={`grid gap-4 rounded-xl border bg-white p-4 shadow-sm transition md:grid-cols-[1fr_140px_120px_auto] md:items-center ${isDeleting ? 'border-red-200 opacity-70' : 'border-slate-200 hover:border-blue-200'}`}>
                    ...
                    <label className="flex flex-col gap-1.5"><span className="text-sm font-black text-slate-700">Requirement</span><select value={document.requirement} onChange={(event) => updateDocument(document.id, 'requirement', event.target.value)} disabled={isBusy || isDeleting} className="h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-50 disabled:cursor-not-allowed disabled:opacity-60"><option value="required">Required</option><option value="optional">Optional</option></select></label>
                    <label className="flex flex-col gap-1.5"><span className="text-sm font-black text-slate-700">Status</span><select value={document.status} onChange={(event) => updateDocument(document.id, 'status', event.target.value)} disabled={isBusy || isDeleting} className="h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-50 disabled:cursor-not-allowed disabled:opacity-60"><option value="active">Active</option><option value="inactive">Inactive</option></select></label>
                    <button type="button" onClick={() => removeDocument(document.id)} disabled={isBusy || isDeleting} className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-red-600 px-4 text-sm font-black text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60">{isDeleting ? <FiLoader className="h-4 w-4 animate-spin" /> : <FiTrash2 className="h-4 w-4" />}{isDeleting ? 'Removing...' : 'Remove'}</button>
                  </div>
                )
              })}
            </section>
        ...
        <div className="flex shrink-0 justify-end border-t border-slate-200 bg-slate-50 px-5 py-4">
          <button type="button" onClick={handleDone} disabled={isBusy} className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-5 text-sm font-black text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60">
            {isSavingDraft ? <><FiLoader className="h-4 w-4 animate-spin" />Saving...</> : <><FiCheckCircle className="h-4 w-4" />Done</>}
          </button>
        </div>
      </div>
    </div>
