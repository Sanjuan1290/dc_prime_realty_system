...
const ListingImportHistoryModal = ({ projectSlug, canUndo = false, onClose }) => {
  ...
  const canReverse = canUndo && reason.trim().length > 0 && !revertMutation.isPending
        ...
        <div className="grid min-h-0 flex-1 lg:grid-cols-[360px_1fr]">
          ...
          <section className="min-h-0 overflow-y-auto p-5">
            ...
            {batch ? <>
              ...
              {!canUndo && importedRows.length ? <div className="mt-4 rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-semibold text-slate-600">Import history is view-only for this account. Undo requires the Listing Import Undo permission.</div> : null}
                ...
                <div className="max-h-[380px] overflow-auto">
                  <table className="w-full min-w-[760px] text-sm"><thead className="sticky top-0 bg-slate-100"><tr><th className="px-3 py-3 text-left"><FiCheckSquare /></th>{['Excel Row','Unit','Status','Import Row State'].map((head) => <th key={head} className="px-3 py-3 text-left text-xs font-black uppercase text-slate-600">{head}</th>)}</tr></thead><tbody className="divide-y divide-slate-100">
                    {rows.map((row) => {
                      ...
                      return <tr key={row.lot_project_listing_import_row_id}><td className="px-3 py-3"><input type="checkbox" disabled={!canUndo || !active} checked={canUndo && active && selectedListingIds.includes(Number(row.lot_project_listing_id))} onChange={() => toggleListing(row.lot_project_listing_id)} /></td><td className="px-3 py-3">{row.excel_row_number}</td><td className="px-3 py-3 font-black">{row.unit_id_snapshot}</td><td className="px-3 py-3">{row.lot_project_listing_status || (active ? 'Unknown' : '-')}</td><td className="px-3 py-3"><span className={`rounded-full px-2 py-1 text-xs font-black ${active ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600'}`}>{row.validation_status}</span></td></tr>
                    })}
                  </tbody></table>
                </div>
              </div>

              {canUndo && importedRows.length ? <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-4">
