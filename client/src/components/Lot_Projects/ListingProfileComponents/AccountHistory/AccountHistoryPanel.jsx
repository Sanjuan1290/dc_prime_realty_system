...
import { FiClock, FiRefreshCw, FiShield, FiTrash2, FiUser } from 'react-icons/fi'
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm font-semibold text-slate-500">Every reservation keeps its buyer, payment, document, cancellation, and commission records.</p>
        </div>
      ...
      {!query.isLoading && !query.isError ? (
        <div className="mt-5 space-y-4">
          {accounts.map((account) => (
            <article key={account.id} className={`rounded-2xl border p-4 ${account.isCurrent ? 'border-blue-200 bg-blue-50/40' : 'border-slate-200 bg-white'}`}>
              <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div className="min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {account.isCurrent ? <span className="rounded-full bg-blue-600 px-3 py-1 text-xs font-black text-white">Current Account</span> : null}
                  </div>
                ...
                {isSuperAdmin && account.status === 'cancelled' && !account.isCurrent ? (
                  <button type="button" onClick={() => setDeleteAccount(account)} className="inline-flex h-10 shrink-0 items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 text-sm font-black text-red-700 transition hover:bg-red-100"><FiTrash2 /> Permanently Delete Records</button>
                ) : null}
              </div>
