...
const HierarchyPreview = ({ preview, isLoading, error, hasSelectedAgent }) => {
  ...
  if (!hasSelectedAgent) return <div className="rounded-xl border border-dashed border-slate-300 p-8 text-center"><p className="font-black text-slate-700">Select a Seller / Group to calculate the commission.</p><p className="mt-1 text-sm font-semibold text-slate-500">In-House recipients or the single External Group recipient will appear here.</p></div>
  if (isLoading) return <div className="flex items-center justify-center gap-2 rounded-xl border border-slate-200 p-8 text-sm font-black text-slate-600"><FiLoader className="animate-spin" />Calculating commission structure...</div>
