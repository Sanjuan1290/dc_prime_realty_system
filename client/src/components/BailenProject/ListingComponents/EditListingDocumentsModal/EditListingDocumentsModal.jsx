...
const libraryDocuments = [
  { id: 4, name: "CLIENT REGISTRATION FORM (Seller's Copy)", description: 'No description' },
  { id: 5, name: 'CLIENT REGISTRATION FORM (Administrator Copy)', description: 'No description' },
  { id: 6, name: "BUYER'S INFORMATION FORM", description: 'No description' },
  { id: 7, name: 'INTENT TO BUY', description: 'No description' },
  { id: 8, name: "OFFER TO BUY & BUYER'S PROFILE", description: 'No description' },
  { id: 9, name: 'RESERVATION AGREEMENT', description: 'No description' },
  { id: 10, name: 'Proof of Billing', description: 'No description' },
  { id: 11, name: 'Proof of Income', description: 'No description' },
]
...
const normalizeDocument = (document) => ({
  ...document,
  description: document.description || 'Project Default',
...
const EditListingDocumentsModal = ({
  ...
  const isDocumentAdded = (documentId) => documents.some((document) => document.id === documentId)
  ...
  const addDocument = (document) => {
    ...
    setDocuments((current) => [
      ...
      normalizeDocument({
        ...document,
        description: 'Custom Listing Requirement',
    ...
    setAlert({
      type: 'success',
      message: `${document.name} added to listing requirements.`,
    })
  }
  ...
  const handleDone = () => {
    ...
    setAlert({ type: 'loading', message: 'Saving draft document requirements...' })

    window.setTimeout(() => {
      setSelectedDocuments?.(documents)
      setIsSavingDraft(false)
      setAlert({ type: 'success', message: 'Draft listing document requirements saved.' })
  ...
  const isBusy = isLoadingDefaults || isSavingDraft
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-slate-950/45 p-4">
      <div className="flex h-[94vh] w-full max-w-6xl flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-2xl">
        <div className="flex h-14 shrink-0 items-center justify-between border-b border-slate-200 px-5">
          <h2 className="text-lg font-black text-slate-950">Edit Documents Before Adding Listing</h2>
          ...
          <button
            ...
            aria-label="Close edit documents modal"
          >
        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <section className="rounded-xl border border-slate-200 bg-slate-50 p-4">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div>
                <h3 className="text-base font-black text-slate-950">Draft Listing Document Requirements</h3>
          ...
          <section className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
            <div className="mb-3 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              ...
              <button
                ...
                disabled={isBusy}
                className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-4 text-sm font-black text-slate-700 transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
              >
            ...
            <div className="relative mb-3">
              ...
              <input
                ...
                className="h-11 w-full rounded-lg border border-slate-300 bg-white pl-9 pr-3 text-sm font-semibold text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-blue-400 focus:ring-4 focus:ring-blue-50"
              />
            </div>
            ...
            <div className="grid max-h-[235px] gap-2 overflow-y-auto pr-1 md:grid-cols-2">
              {filteredLibraryDocuments.map((document) => {
                ...
                return (
                  <div
                    key={document.id}
                    className="flex items-center justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 p-3 transition hover:border-blue-200 hover:bg-blue-50/40"
                  >
          ...
          {documents.length ? (
            <section className="mt-4 space-y-2">
              {documents.map((document) => {
                ...
                return (
                  <div
                    key={document.id}
                    className={`grid gap-4 rounded-xl border bg-white p-4 shadow-sm transition md:grid-cols-[1fr_140px_120px_auto] md:items-center ${
                      isDeleting ? 'border-red-200 opacity-70' : 'border-slate-200 hover:border-blue-200'
                    }`}
                  >
                    <div className="min-w-0">
                      <p className="break-words text-base font-black text-slate-950">{document.name}</p>
                      <p className="mt-1 text-sm font-semibold text-slate-500">{document.description}</p>
                      <p className="mt-1 text-xs font-semibold text-slate-400">Source: {document.source}</p>
                    </div>

                    <label className="flex flex-col gap-1.5">
                      <span className="text-sm font-black text-slate-700">Requirement</span>
                      <select
                        value={document.requirement}
                        onChange={(event) => updateDocument(document.id, 'requirement', event.target.value)}
                        disabled={isBusy || isDeleting}
                        className="h-11 rounded-lg border border-slate-300 bg-white px-3 text-sm font-semibold text-slate-700 outline-none transition focus:border-blue-400 focus:ring-4 focus:ring-blue-50 disabled:cursor-not-allowed disabled:opacity-60"
                      >
                    ...
                    <button
                      ...
                      disabled={isBusy || isDeleting}
                      className="inline-flex h-11 items-center justify-center gap-2 rounded-lg bg-red-600 px-4 text-sm font-black text-white transition hover:bg-red-700 disabled:cursor-not-allowed disabled:opacity-60"
                    >
                      {isDeleting ? (
            ...
            <div className="mt-4 rounded-xl border border-dashed border-slate-300 bg-white p-5 text-sm font-semibold text-slate-500">
        ...
        <div className="flex shrink-0 justify-end border-t border-slate-200 bg-slate-50 px-5 py-4">
          <button
            ...
            onClick={handleDone}
            disabled={isBusy}
            className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-slate-300 bg-white px-5 text-sm font-black text-slate-700 shadow-sm transition hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isSavingDraft ? (
              <>
                <FiLoader className="h-4 w-4 animate-spin" />
