...
test('attendance export endpoint and workbook generation are wired into Attendance', () => {
  ...
  assert.match(controller, /breakStart: '12:00:00'/)
  ...
  assert.match(exportModal, /Scheduled Time In/)
  assert.match(exportModal, /Break Duration \(minutes\)/)
  assert.match(exportModal, /Highlight Row Red After/)
  assert.match(exportModal, /Editable for this export only/)
  assert.match(workbook, /xlsx-js-style/)
  assert.match(workbook, /RD OT/)
  assert.match(workbook, /breakOverlapSeconds/)
  assert.match(workbook, /schedule\.lateAfter/)
