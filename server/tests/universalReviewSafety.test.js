...
test('only free-form text, number, and textarea controls get examples while Proceed treatment remains global', () => {
  ...
  assert.doesNotMatch(provider, /custom\.\*monthly\.\*term/);
  assert.match(provider, /getAttribute\('data-example'\)/);
  assert.match(provider, /if \(explicitExample\) return explicitExample/);
  assert.match(provider, /An authored placeholder is safer than a global keyword guess/);
  assert.doesNotMatch(provider, /if \(\/rate\|percentage\|percent/);
  assert.match(reserveTerms, /<option value="custom">Custom<\/option>/);
  assert.match(reserveTerms, /Custom Daily Penalty Rate \(%\)[\s\S]*?type="number"[\s\S]*?example="0\.15%"/);
  assert.match(reserveTerms, /Custom Downpayment Percentage[\s\S]*?example="20%"/);
  assert.match(reserveTerms, /Custom Monthly Terms[\s\S]*?example="30 months"/);
