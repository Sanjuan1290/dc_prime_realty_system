...
export const getSystemReports = async (req, res) => {
  ...
  try {
    ...
    // Commission reporting follows the exact same sales cohort as the Sales bridge:
    // only commissions created by reservations/sales inside the selected range belong
    // to this report's commission reconciliation. This prevents a release from an old
    // sale from being mixed with commission generated by the current sales cohort.
    const cohortCommissionIds = new Set(commissions.map((row) => Number(row.id || 0)).filter(Boolean))
    const commissionCohortReleases = releases.filter((row) => cohortCommissionIds.has(Number(row.commissionId || 0)))
    const cohortReleased = commissionCohortReleases.filter((row) => {
    ...
    const ensureSeller = ({ sellerId = null, seller = '-', sellerGroup = '-' } = {}) => {
      ...
      if (!sellerPerformanceMap.has(key)) {
        sellerPerformanceMap.set(key, {
          ...
          netCashMovement: 0,
          grossCommission: 0,
          commissionForfeited: 0,
          commissionCancelled: 0,
          commissionDeductions: 0,
