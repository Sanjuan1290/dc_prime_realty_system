...
const normalizePayload = (body = {}) => {
  const amount = roundMoney(positiveNumber(body.amount));
  const installmentCount = Math.min(Math.max(Math.trunc(positiveNumber(body.installment_count, 1)), 1), 60);
  const requestedDeduction = roundMoney(positiveNumber(body.deduction_per_payroll));
  return {
    employeeId: Number(body.employee_id || 0),
    requestDate: dateOnly(body.request_date),
    amount,
    installmentCount,
    deductionPerPayroll: requestedDeduction > 0 ? Math.min(requestedDeduction, amount) : roundMoney(amount / installmentCount),
    startDeductionDate: dateOnly(body.start_deduction_date),
  ...
  try {
    ...
    return res.json({
      ...
      data: rows.map((row) => ({
        ...
        deduction_per_payroll: Number(row.deduction_per_payroll || 0),
...
export const getEmployeeCashAdvance = async (req, res) => {
  ...
  try {
    ...
    return res.json({ success: true, data: { ...rows[0], deductions } });
  } catch (error) {
    return res.status(500).json({ message: getErrorMessage(error) });
  } finally {
    connection.release();
...
export const createEmployeeCashAdvance = async (req, res) => {
  ...
  try {
    ...
    if (!payload.employeeId || !payload.requestDate || !payload.startDeductionDate || payload.amount <= 0) {
      return res.status(400).json({ message: 'Employee, request date, start deduction date, and amount are required.' });
    }
    ...
    const [result] = await connection.query(
      ...
      `,
      [payload.employeeId, reference, payload.requestDate, payload.amount, payload.installmentCount, payload.deductionPerPayroll, payload.startDeductionDate, payload.amount, payload.notes, req.authUser?.id || null, req.authUser?.id || null]
    );
    await writeAuditLog(connection, req, {
      ...
      metadata: { employeeId: payload.employeeId, amount: payload.amount, installmentCount: payload.installmentCount },
    });
...
export const updateEmployeeCashAdvance = async (req, res) => {
  ...
  try {
    ...
    if (!payload.employeeId || !payload.requestDate || !payload.startDeductionDate || payload.amount <= 0) return res.status(400).json({ message: 'Complete the required cash advance fields.' });
    await connection.query(
      ...
      [payload.employeeId, payload.requestDate, payload.amount, payload.installmentCount, payload.deductionPerPayroll, payload.startDeductionDate, payload.amount, payload.notes, req.authUser?.id || null, advanceId]
    );
    await writeAuditLog(connection, req, {
      actor: req.authUser,
      ...
      description: `Updated ${rows[0].reference_number}.`, metadata: { amount: payload.amount, installmentCount: payload.installmentCount },
    });
...
const changeStatus = async (req, res, nextStatus) => {
  ...
  try {
    ...
    await connection.query(
      `UPDATE employee_cash_advances SET cash_advance_status = ?, approved_by_user_id = ?, approved_at = ?, updated_by_user_id = ? WHERE employee_cash_advance_id = ?`,
      [storedStatus, nextStatus === 'approved' ? req.authUser?.id || null : advance.approved_by_user_id, nextStatus === 'approved' ? new Date() : advance.approved_at, req.authUser?.id || null, advanceId]
    );
    await writeAuditLog(connection, req, {
      actor: req.authUser,
      action: 'update', module: 'Employee Cash Advances', entityType: 'employee_cash_advance', entityId: String(advanceId),
